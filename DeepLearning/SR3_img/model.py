...
class Unet(nn.Module):
    def __init__(self, in_channels = 1, pos_encoding_dim = 100):
        super().__init__()
        self.pos_encoding_dim = pos_encoding_dim
        self.down1 = ConvBlock(in_channels*2, 64, self.pos_encoding_dim)
    ...
    def low_resolution(self, x):
        y = self.pool(self.pool(x))
        y = self.upsample(self.upsample(y))
        return y

    def forward(self, x, ts, y):
...
class Diffuser:
    ...
    def sampling(self, model, high_x, return_img = False):
        x_shape = high_x.shape
        y = model.low_resolution(high_x)
